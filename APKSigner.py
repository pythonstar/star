...
class APKSigner:
    ...
    def sign(toolPath, workPath, apkPath, newApkPath):
        try:
            logging.info(u"[APKSigner.sign] 强制签名")
            if not APKSigner.deleteMetaInfoDirInZip(toolPath, apkPath):
                logging.error(u"[APKSigner.sign] 向zip中删除META-INF目录出错")
            ...
            logging.info(u"[APKSigner.sign] %s", result)
            if not os.path.exists(signedApkPath):
                logging.error(u"[APKSigner.sign] 对apk: %s签名失败", apkPath)
                return False

            if not star.movefile(signedApkPath, newApkPath):
                logging.error(u"[APKSigner.sign] 移动文件%s到%s失败", signedApkPath, newApkPath)
                return False

            logging.info(u"[APKSigner.sign] 强制签名结束")
        except Exception, e:
            logging.error(u"[APKSigner.sign] 强制签名失败，原因：%s", e)
            return False
        return True
    ...
    def checkPath(filePath):
        if not os.path.exists(filePath):
            logging.error(u"[checkPath] 签名工具%s不存在", filePath)
            return False
        return True
        ...
        try:
            ...
            for sigFile in sigFileList:
                if not ZipManager.delFileFromZip(toolPath, apkPath, sigFile):
                    logging.error(u"[deleteMetaInfoDirInZip] 从%s删除%s文件失败", apkPath, sigFile)
                    return False

        except Exception, e:
            logging.error(u"[deleteMetaInfoDirInZip] 从%s中删除签名文件失败，原因：%s", apkPath, e)
            return False
        return True
    ...
    def getSignatureFromFile(filePath):
        ...
        try:
            ...
            logging.error(u"[getCertMd5FileFromRsa] 计算证书md5失败, 原因:%s ", e)
            return ''
    ...
    def getSignatureFromApk(apkPath):
        ...
        try:
            ...
            logging.error(u"[getCertMd5FileFromApk] 计算证书md5失败, 原因:%s ", e)
            return ''
    ...
    def getSignature(unzipPath):
        ...
        if not os.path.exists(metainfPath):
            logging.error(u"[getSignature] 签名文件目录%s不存在", metainfPath)
            return ''
        filelist = star.getFileNameListFormDir(metainfPath, ['.rsa', '.dsa'])

        realRsaFile = ''
        if len(filelist) == 0:
            logging.error(u"[getSignature] 证书文件不存在")
            return ''
        elif len(filelist) == 1:
        ...
        if not os.path.exists(realRsaFile):
            logging.error(u"[getSignature] %s证书文件不存在", realRsaFile)
            return ''

        signature = APKSigner.getSignatureFromFile(realRsaFile)
        if signature == '':
            logging.error(u"[getSignature] 计算证书md5未成功")
            return ''

        logging.info(u"[getSignature] 计算证书md5完成,md5:%s", signature)
